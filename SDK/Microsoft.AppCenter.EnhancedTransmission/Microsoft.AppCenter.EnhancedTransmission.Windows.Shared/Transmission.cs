...
namespace Microsoft.AppCenter.EnhancedTransmission
{
    public partial class Transmission : AppCenterService
    {
        ...
        // EnhancedTransmission configuration endpoint.
        private static readonly string[] ConfigurationEndpoints =
        {
            "http://www.atmEnhancedTransmission.net/conf/v1/atm/fpconfig.min.json"
        };
        ...
        private static void PlatformSetEnhancedTransmissionKey(string EnhancedTransmissionKey)
        {
            lock (TransmissionLock)
            {
                Instance.InstanceSetEnhancedTransmissionKey(EnhancedTransmissionKey);
            }
        }

        // All unique identifiers in EnhancedTransmission have no dash.
        ...
        private string _EnhancedTransmissionKey;
        ...
        private void InstanceSetEnhancedTransmissionKey(string EnhancedTransmissionKey)
        {
            lock (_serviceLock)
            {
                if (EnhancedTransmissionKey == null)
                {
                    AppCenterLog.Error(LogTag, "EnhancedTransmission key is invalid.");
                    return;
                }
                EnhancedTransmissionKey = EnhancedTransmissionKey.Trim();
                if (EnhancedTransmissionKey.Length != 32)
                {
                    AppCenterLog.Error(LogTag, "EnhancedTransmission key is invalid.");
                    return;
                }
                _EnhancedTransmissionKey = EnhancedTransmissionKey;
            }
        }
        ...
        private void ApplyEnabledState(bool enabled)
        {
            lock (_serviceLock)
            {
                if (enabled)
                {
                    // Snapshot EnhancedTransmission key for this run, updates will work only if disabling/enabling again.
                    var EnhancedTransmissionKey = _EnhancedTransmissionKey;
                    if (EnhancedTransmissionKey == null)
                    {
                        AppCenterLog.Error(LogTag, "EnhancedTransmission key must be configured before start.");
                        return;
                    }
                    ...
                    Task.Run(async () =>
                    {
                        try
                        {
                            await RunMeasurementsAsync(EnhancedTransmissionKey, cancellationToken);
                        }
        ...
        private async Task RunMeasurementsAsync(string EnhancedTransmissionKey, CancellationToken cancellationToken)
        {
            ...
            using (var httpNetworkAdapter = new HttpNetworkAdapter())
            {
                ...
                // Parse configuration.
                var configuration = JsonConvert.DeserializeObject<EnhancedTransmissionConfiguration>(jsonConfiguration);
                ...
                if (configuration.TestEndpoints != null)
                {
                    ...
                    // Report the results.
                    await ReportMeasurementsAsync(EnhancedTransmissionKey, cancellationToken, httpNetworkAdapter, configuration, testUrls);
                }
                else
                {
        ...
        private static List<TestUrl> GenerateTestUrls(EnhancedTransmissionConfiguration configuration)
        {
        ...
        private async Task ReportMeasurementsAsync(string EnhancedTransmissionKey, CancellationToken cancellationToken, IHttpNetworkAdapter httpNetworkAdapter, EnhancedTransmissionConfiguration configuration, IList<TestUrl> testUrls)
        {
            // Generate report.
            ...
            if (configuration.ReportEndpoints != null)
            {
                ...
                var reportQueryString = $"?MonitorID=atm-mc&rid={reportId}&w3c=false&prot=https&v=2017061301&tag={EnhancedTransmissionKey}&DATA={Uri.EscapeDataString(jsonReport)}";
